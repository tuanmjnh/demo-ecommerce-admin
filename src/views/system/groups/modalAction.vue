...
const onReset = () => {
  ...
  if (!modelForm.value.code) modelForm.value.code = viToSlug(NewGuid()).toUpperCase()
...
watch(() => modelForm.value.code, debounce(async (n) => {
  if (!n) return
  modelForm.value.code = viToSlug(n).toUpperCase()
}, 600))
...
const onSubmit = () => {
  modelFormRef?.value?.validate((errors: any) => {
    if (!errors) {
      try {
        modelForm.value.code = viToSlug(modelForm.value.code).toUpperCase()
        ...
        <n-grid :cols="2">
          ...
          <n-form-item-gi :span="2" :label="$t('components.groups.dependent')" path="parent">
            ...
            <n-tree-select :options="groups" key-field="_id" label-field="title" :default-value="modelForm.parent"
              :default-expanded-keys="[modelForm.parent]" :placeholder="$t('common.default')"
          ...
          <n-form-item-gi :span="1" :label="$t('common.avatar')">
            <div>
              <MediaGalleryLinks v-model="modelForm.image" class="mb-2" quaternary size="250x160" maxHeight="340px"
              ...
              <MediaGallery v-model="modelForm.image" size="220x160" show-name
          ...
          <n-form-item-gi :span="1" :label="$t('common.attributes')" path="meta">
