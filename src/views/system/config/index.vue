...
import { useAppStore, useLoadingStore, useConfigStore } from '@/stores'
...
const items = computed(() => configStore.items)
const item = ref<Models.IConfig | null>(null)
...
const columns = ref<any[]>([
  ...
  },
  {
    ...
    sortOrder: false,
    sorter: 'default',
    render: (row: any) => toUpperCaseFirst(row.type)
  },
  {
...
const onFetch = async (isResetPage?: boolean) => {
  ...
  await configStore.get(query.value)
}
onMounted(() => {
  onFetch()
})
...
const onConfirmFlag = async (row?: any) => {
  ...
  if (row) rows = [row._id]
  const res = await configStore.updateFlag(rows, viewType.value === 1 ? 0 : 1)
  if (res.status) {
    checkedRows.value = []
...
<template>
  <n-space vertical>
    <n-card :title="$t('components.config.title')">
          ...
          <n-tooltip trigger="hover">
            <template #trigger>
              <n-button type="primary" text-color="#fff" attr-type="button" block
                @click="modalActionRef.openModal('add', { ...configStore.getDefaultModel, ...{ key: query.key } })">
    ...
    <modal-action ref="modalActionRef" :data="item" :data-default="configStore.getDefaultModel" :key-option="query.key"
      :title="$t('components.config.title')" @on-close="() => item = null" />
  </n-space>
</template>
