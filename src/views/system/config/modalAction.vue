...
import { useLoadingStore, useAppStore, useConfigStore } from '@/stores'
...
const {
  ...
  title?: string
  data: Models.IConfig | null
  dataDefault: Models.IConfig | null
}>()
...
async function openModal(type: ModalType = 'add', data: Models.IConfig) {
  emit('onOpen')
...
watch(() => modelForm.value.code, debounce(async (n) => {
  if (!n) return
  modelForm.value.code = viToAscii(n).toLowerCase()
}, 600))

const onSubmit = async () => {
  modelFormRef?.value?.validate(async (errors: any) => {
    if (!errors) {
      try {
        if (modalType.value == 'add') {
          await configStore.create(JSON.parse(JSON.stringify(modelForm.value))).then(x => {
            if (x.status) {
          ...
          await configStore.update(JSON.parse(JSON.stringify(modelForm.value))).then(x => {
            if (x.status) {
      ...
      <n-form ref="modelFormRef" label-placement="left" :model="modelForm" :label-width="150">
        <n-grid :cols="2" :x-gap="18">
          <n-form-item-gi :span="1" :label="$t('common.title')" path="title"
            ...
            <n-input v-model:value="modelForm.title" v-text-format:first :disabled="modalType == 'edit'"
              :placeholder="$tm(['common.pleaseInput', 'common.title'])" />
          </n-form-item-gi>
          <n-form-item-gi :span="1" :label="$t('common.type')" path="type"
            :rule="[{ required: true, message: $tm(['common.pleaseInput', 'common.type']), trigger: ['blur', 'change', 'input'] }]">
            <n-select v-model:value="modelForm.type" :options="configStore.getConfigTypeOptions" filterable
          ...
          <n-form-item-gi :span="1" :label="$t('common.code')" path="code" :rule="[
            { required: true, message: $tm(['common.pleaseInput', 'common.code']), trigger: ['blur', 'change', 'input'] },
            {
              asyncValidator: async (r, v) => {
                if (await configStore.exist({ filter: { code: v }, id: modelForm._id })) throw new Error($t('message.error.existCode'))
              }, trigger: ['blur']
            }
          ]">
            <n-input v-model:value="modelForm.code" v-text-format:lower :disabled="modalType == 'edit'"
              :placeholder="$tm(['common.pleaseInput', 'common.code'])" />
          </n-form-item-gi>
          <n-form-item-gi :span="1" :label="$t('common.value')" path="value"
            :rule="[{ required: true, message: $tm(['common.pleaseInput', 'common.value']), trigger: ['blur', 'change', 'input'] }]">
            <n-input v-model:value="modelForm.value" v-text-format:lower
              :placeholder="$tm(['common.pleaseInput', 'common.value'])" />
          </n-form-item-gi>
          <n-form-item-gi :span="2" :label="$t('common.desc')" path="desc">
            <n-input v-model:value="modelForm.desc" v-text-format:first :disabled="modalType == 'edit'"
