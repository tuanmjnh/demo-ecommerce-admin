...
import { useAppStore, useLoadingStore, useRouteStore, useRoleStore } from '@/stores'
...
const item = ref<Models.IRole | null>(null)
const routes = ref<Models.RouteItem[]>(routeStore.getRoutes)
const routesTree = ref<Models.RouteItem[]>(routeStore.getRoutesTreeDisable)
...
const onConfirmFlag = async (row?: any) => {
  ...
  if (row) rows = [row._id]
  const res = await roleStore.updateFlag(rows, viewType.value === 1 ? 0 : 1)
  if (res.status) {
    checkedRows.value = []
...
<template>
  <n-space vertical>
    <n-card :title="$t('components.roles.title')">
          ...
          <n-tooltip trigger="hover">
            <template #trigger>
              <n-button type="primary" text-color="#fff" attr-type="button" block
                @click="modalActionRef.openModal('add', roleStore.getDefaultModel)">
    ...
    <modal-action ref="modalActionRef" :data="item" :data-default="roleStore.getDefaultModel" :routes="routes"
      :routes-tree="routesTree" :title="$t('components.roles.name')" @on-close="() => item = null" />
  </n-space>
</template>
