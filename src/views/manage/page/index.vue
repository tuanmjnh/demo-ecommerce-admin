...
import { $t, renderLabelSelect, renderActionButtons, renderTooltipCell } from '@/utils'
  ...
  // },
  {
    ...
    render: (row: any) => renderActionButtons(row, [
      ...
      },
      {
        ...
        type: 'info',
        onClick: (r) => onCopy(r)
        //modalActionRef.value.openModal('add', { ...r, _id: null, code: null, title: `${r.title} - copy`, flag: 0 })
      },
      {
...
const onConfirmFlag = async (row?: any) => {
  ...
  if (row) rows = [row._id]
  const res = await pageStore.updateFlag(rows, viewType.value === 1 ? 0 : 1)
  if (res.status) {
    checkedRows.value = []
...
<template>
  <n-space vertical>
    <n-card :title="$t('components.page.title')">
    ...
    <!-- <modal-action ref="modalActionRef" :data="item" :data-default="pageStore.getDefaultModel" :title="$t('route.post')"
