...
import { useAppStore, useLoadingStore, useGroupStore, useProductStore } from '@/stores'
...
const groupType = route.meta.group || 'billing-items'
const query = ref({
  text: '',
  groups: [],
  key: groupType,
...
const item = ref<Models.IProduct | null>(null)
...
const columns = ref<any[]>([
  ...
  {
    title: () => h('span', `${$t('components.product.priceSelling')} (vnđ)`),
    key: 'priceSelling',
    sortOrder: false,
    sorter: 'default',
    render: (row: any) => h('span', row.priceSelling.toLocaleString())
  },
  // {
  //   title: () => h('span', $t('components.product.pricePromotional')),
  //   key: 'pricePromotional',
  //   sortOrder: false,
  //   sorter: 'default'
  // },
  {
...
const onFetch = async (isResetPage?: boolean) => {
  ...
  await productStore.get(query.value)
}
onMounted(() => {
  onFetch()
  groupStore.filter({ key: groupType, flag: 1 })
  // groupStore.getAll({ key: 'product' })
  // optionsStore.getAll({ key: 'unit' })
})
...
const onConfirmFlag = async (row?: any) => {
  ...
  if (row) rows = [row._id]
  const res = await productStore.updateFlag(rows, viewType.value === 1 ? 0 : 1)
  if (res.status) {
    checkedRows.value = []
...
<template>
  <n-space vertical>
    <n-card :title="$t('components.news.title')">
      <template #header-extra>
        <n-space>
          <n-input v-model:value="query.text" :placeholder="$t('common.searchInput')" @input="onSearch" />
          <n-select :options="(groups as any)" value-field="_id" label-field="title" :placeholder="$t('common.default')"
            multiple checkable virtual-scroll class="w-380px"
          ...
          <n-tooltip trigger="hover">
            <template #trigger>
              <n-button type="primary" text-color="#fff" attr-type="button" block
                @click="modalActionRef.openModal('add', { ...{}, ...productStore.getDefaultModel })">
    ...
    <modal-action ref="modalActionRef" :data="item" :data-default="productStore.getDefaultModel"
      :title="$t('route.product')" :groups="groups" @on-close="() => item = null" />
  </n-space>
</template>
