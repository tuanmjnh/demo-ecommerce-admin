...
const items = computed(() => groupStore.getTreeGroups)
const item = ref<Models.IGroup | null>(null)
...
const onEdit = async (item: Models.IGroup) => {
  groupStore.setItem(item)
  router.push(`edit/${item._id}`)
}
const onCopy = async (item: Models.IGroup) => {
  groupStore.copyItem(item)
  router.push(`add`)
}

const onFetch = async (isResetPage?: boolean) => {
  if (isResetPage) query.value.page = 1
  await groupStore.filter(query.value)
}
onMounted(() => {
  onFetch()
})
...
const onConfirmFlag = async (row?: any) => {
  ...
  if (row) rows = [row._id]
  const res = await groupStore.updateFlag(rows, viewType.value === 1 ? 0 : 1)
  if (res.status) {
    checkedRows.value = []
...
<template>
  <n-space vertical>
    <n-card :title="$t(`components.groups.${groupType}`)">
      <template #header-extra>
        <n-space>
          <n-input v-model:value="query.text" :placeholder="$t('common.searchInput')" @input="onSearch" />
          <n-tooltip trigger="hover">
            <template #trigger>
              <n-button type="primary" text-color="#fff" attr-type="button" block @click="onAdd">
    ...
    <modal-action ref="modalActionRef" :data="item" :data-default="groupStore.getDefaultModel" :group-type="groupType"
      :title="$t(`route.${groupType}Groups`)" @on-close="() => item = null" />
  </n-space>
</template>
