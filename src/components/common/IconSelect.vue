...
<template>
  <n-input-group disabled>
    <n-button v-if="value" text-color="#fff" :disabled="disabled" type="primary">
      <template #icon>
        <nova-icon :icon="value" />
      </template>
    </n-button>
    <n-input v-if="showInput" :value="value" readonly :placeholder="$t('components.iconSelector.inputPlaceholder')" />
      ...
      <n-tab-pane name="local" tab="local">
        <n-flex :size="2">
          <n-el v-for="(_icon, key) in LocalIconList" :key="key"
            class="hover:(text-[var(--primary-color)] ring-1) ring-(--primary-color) p-1 rounded flex-center"
