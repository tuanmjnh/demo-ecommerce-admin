...
<template>
  <n-layout class="wh-full" embedded>
    <n-layout class="h-full flex flex-col" content-style="display: flex;flex-direction: column;min-height:100%;"
      ...
      <Footer :visible="appStore.showFooter && !appStore.contentFullScreen" :text="appStore.footerText" />
      <!-- <n-layout-footer v-if="appStore.showFooter && !appStore.contentFullScreen" bordered
        position="absolute" class="h-40px flex-center">
        {{ appStore.footerText }}
        </n-layout-footer> -->
      <BackTop />
    </n-layout>
