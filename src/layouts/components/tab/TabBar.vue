  ...
  <n-scrollbar ref="scrollbar" class="relative flex h-full tab-bar-scroller-wrapper"
    ...
    <div class="p-l-2 flex wh-full relative">
      <div class="flex items-end">
        <TabBarItem v-for="item in tabStore.pinTabs" :key="item.fullPath" :value="tabStore.currentTabPath"
          :route="(item as any)" @click="handleTab((item as any))" />
      </div>
      <div ref="el" class="flex items-end flex-1">
        <TabBarItem v-for="item in tabStore.tabs" :key="item.fullPath" :value="tabStore.currentTabPath"
          :route="(item as any)" closable :data-tab-path="item.fullPath" @close="tabStore.closeTab"
          @click="handleTab((item as any))" @contextmenu="handleContextMenu($event, (item as any))" />
